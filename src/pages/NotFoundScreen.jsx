...
const NotFoundScreen = ({ navigate }) => (
  <WebLayout navigate={navigate} currentPage="">
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flex: 1, minHeight: "60vh" }}>
      <div style={{ maxWidth: 480, textAlign: "center" }}>
        ...
        <div style={{ background: "rgba(255,59,59,0.06)", border: "1px solid rgba(255,59,59,0.2)", borderRadius: 10, padding: "14px 16px", marginBottom: 28 }}>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--red)", marginBottom: 6 }}>TRACE LOG</div>
          {["Route lookup failed: /unknown", "No matching encrypted tunnel found", "Connection terminated ✗"].map((line, i) => (
            <div key={i} className="terminal-line" style={{ color: "var(--red)", opacity: 0.7, animationDelay: `${i * 0.4}s`, marginBottom: 3 }}>{line}</div>
          ))}
        </div>
